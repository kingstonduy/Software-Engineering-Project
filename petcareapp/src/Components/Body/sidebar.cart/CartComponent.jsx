...
const CartComponent = () => {
    ...
    const handlePlusClick = async  (product) => {
        console.log(product);
        await cartContext.updatePlusQuantityItemOnCart(product);
    };
        ...
        <div className={cs.container}>
            <div className={`${cs['hidden-menu']} ${isOpen ? cs.open : ''}`}>
                <div className={cs['header']}>
                    ...
                    <h3 className={cs['title']}>Shopping cart</h3>
                </div>
                                        ...
                                        <div className={cs['table-row']} key={v.id}>
                                            ...
                                            <div className={cs['product-main-group']}>
                                                <div className={cs['product-name']}>
                                                    {v.cartDTOName.length > 30
                                                        ? `${v.cartDTOName.slice(0, 30)}...`
                                                        : v.cartDTOName}
                                                </div>
                                                ...
                                                <div className={cs['product-action']}>
                                                    <div className={cs['add-minus-btn']}>
                                                        <button
                                                            className={cs['hidden-menu-btn']}
                                                            onClick={() => handleMinusClick(v)}
                                                        >
                                                            <FontAwesomeIcon icon={faMinus} />
                                                        </button>

                                                        <input
                                                            className={cs['input-quantity']}
                                                            autocomplete="off"
                                                            value={v.cartDTOQuantity}
                                                            type="number"
                                                            inputmode="numeric"
                                                            name="updates[]"
                                                            data-line="1"
                    ...
                    <center>
                        <button className={cs['checkout-div']} onClick={moveToCheckout}>
                            <Link to="/Checkout" className={cs['checkout-btn']}>
                                <span className={cs['check-out-link']}>Check out</span>
                            </Link>
                        </button>
                    </center>
                )}
